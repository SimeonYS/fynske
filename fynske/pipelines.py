...
class FynskePipeline:

    # Database setup
    conn = sqlite3.connect('fynske.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fynske`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fynske WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fynske (date, title, link, content)"
